...
- The tube is beamed to the engine block corner nodes (`e1l…e4r` — the vanilla naming
  ...
  snorkel off.
- The meshes (`snorkel.dae`, objects `snorkel_small/medium/tall`) are generated by
  `tools/generate_snorkel_dae.py` — a parametric tube + ram head + brackets. Adjust the
  path constants and re-run to move/resize the snorkel.
